...
async function largeFileUploadTest(
  browser: Browser,
  ...
  const errorMessage = await page.$eval(
    "body",
    (el) =>
      el.innerText.includes("file is too large") ||
      el.innerText.includes("size limit exceeded")
  );
  ...
  return {
    ...
    failedSteps: errorMessage
      ? []
      : ["No error message found for large file upload"],
    details: errorMessage
      ? "Large file upload was correctly rejected"
      : "Large file upload did not produce expected error",
  };
}
