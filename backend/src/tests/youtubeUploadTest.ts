...
async function youtubeUploadTest(
  ...
  const errorMessage = await page.$eval(
    "body",
    (el) =>
      el.innerText.includes("YouTube uploads are not supported") ||
  ...
  return {
    ...
    failedSteps: errorMessage
      ? []
      : ["No error message found for YouTube upload"],
    details: errorMessage
      ? "YouTube upload was correctly rejected"
      : "YouTube upload did not produce expected error",
  };
}
